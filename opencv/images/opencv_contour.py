...
#contour 는 연결된 외곽선이다.
#edge 가 경계선이라면
#contour 는 경계선을 따라 연결된 객체의 외곽선 묶음이다.
#threshold 나 edge 로 동전과 배경으 분리하고 contour로 찾는다

import cv2
img = cv2.imread("opencv/images/coin.png")

gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)

blur = cv2.GaussianBlur(gray, (5,5) , 0 )

#cv2.THRESH_BINARY_INV 를 한 이유는 배경이 밝은색이고
# 찾고자 하는 동전이 어둡기 때문이다.
#배경이 어둡다면, cv2.THRESH_BINARY 로 하면된다.
...
# 발견된 영역에서  가장 바깥쪽 외곽선만찾는다.
# 동전의 가운데가 검은색으로 표시되도 바깥쪽 외곽만 사용한다.
...
# findContours 함수의 두번째 반환값은계층 정보를 가지게 된다.
# 두번째 반환값으로 [next(같은계층 contour), pre(같은계층의 이전 contour)]
#                   first_child (안쪽계층의 첫번째 자식 contour),
#                   parent( 현재 contour 의 부모 contour)

#findContours함수 3번째인자 method
#   cv2.CHAIN_APPROX_NONE : 외곽선의 모든 픽셀 좌표저장 (정밀분석)
#   cv2.CHAIN_APPROX_TC89_L1 : teh-chain 알고리즘 기반 - 
...
#drawContours ( 이미지 , contours, 어떤contour에 그리는가?, 색상, 선굵기 )
