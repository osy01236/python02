...
def create_report_service(db):
    ...
    # TTFont("Malgun Gothic", "C:/Windows/Fonts/malgun.ttf")
    # )
    ...
    styles["BodyText"].fontName = "HYSMyeongJo-Medium"



    story = []
    title = Paragraph("학생 학습 분석 리포트", styles["Title"])
    story.append(title)
    story.append(Spacer(1, 24))

    summary_data = get_class_summary(db)
    story.append(Paragraph("1. 반 전체 통계", styles["Heading1"]))
    story.append(Paragraph(f"전체 학생 수 : {summary_data['total_student']}명", styles["BodyText"]))
    story.append(Paragraph(f"평균 점수 : {summary_data['avg_score']:.2f}", styles["BodyText"]))
    story.append(Paragraph(f"평균 출석률 : {summary_data['avg_attend_rate']:.2f}%", styles["BodyText"]))
    story.append(Spacer(1, 12))

    student_stat = get_student_status_service(db)

    story.append(Paragraph("2. 학생 상태 분석.", styles["Heading1"]))
    story.append(Spacer(1, 10))        
    for std in student_stat["danger_std"]:
        text=(f"{std['name']} /"
            f"평균점수 : {std['avg_score']:.2f} /" 
            f"출석률 : {std['attend_rate']:.2f}%")
        story.append(Paragraph(text, styles["BodyText"]))
        
    story.append(Spacer(1, 10))

    story.append(Paragraph("3. 우수 학생", styles["Heading1"]))
    story.append(Spacer(1, 10))     

    for std in student_stat["excellent_std"]:
        text=(f"{std['name']} /"
            f"평균점수 : {std['avg_score']} /" 
            f"출석률 : {std['attend_rate']}")
        story.append(Paragraph(text, styles["BodyText"]))

    story.append(PageBreak())
    story.append(Paragraph("4. 분석 그래프", styles["Heading1"]))
    ...
    story.append(Spacer(1, 10)) # Spacer - pdf 줄바꿈 및 여백 (몇줄 , 아래쪽 여백 열마 (pt))
    story.append(
        Paragraph(
            f"평균 점수 : {graph_summary['avg_scores']}점 /"
            f"최고 점수 : {graph_summary['max_score']}점 /"
            f"최저 점수 : {graph_summary['min_score']}점 /"
            f"표준 편차 : {graph_summary['std_score']} ",
            styles["BodyText"]
        )
    )

    story.append(Spacer(1, 20))
    story.append(Paragraph("5. 출석률 그래프", styles["Heading1"]))
    story.append(Spacer(1, 10))
    ...
    story.append(
        Paragraph(
            ...
            f"표준 편차 : {graph_summary['std_rate']} ",
            styles["BodyText"] 
        )
    )
...
def get_graph_summary_service(db):
    ...
    for attend in attends:
        tot_cnt = ( #학생의 출력률
            attend.attend + attend.late + attend.absent + attend.early_leave
        )
    ...
    return {
        "avg_scores" : round(np.mean(avg_scores), 2),
...
def score_graph_make(db):
    ...
    labels = ["0~50점", "50~60점", "60~70점", "70~80점", "80~90점", "90~100점"]
    ...
    ax.set_title("성적 분포 그래프", fontsize=16, fontweight="bold", pad=20)

    ax.set_ylabel("학생 수(명)", fontsize=11)
...
def attend_graph_make(db):
    ...
    attend_counts = df["attend_rate_range"].value_counts().sort_index()
    colors=["#47C83E",
            "#368AFF",
            "#FFCD12",
            "#FF5A5A"
    ...
    legend_labels=[]
    for label , value in zip(labels, attend_counts):
    ...
    ax.set_title("출석률 분포", fontsize=26, fontweight="bold", pad=20)
...
def get_student_status_service(db):
    ...
    for std in students:
        score = db.query(Score).filter(Score.student_id == std.id).order_by(Score.id.desc()).first()
        attend = db.query(Attend).filter(Attend.student_id == std.id).order_by(Attend.id.desc()).first()

        if score is None or attend is None:
            continue
...
def get_class_summary(db):
    ...
    temp_avg_score = 0 #전체 평균 계산위한 임시 변수
    for score in scores:
        avg = (score.python_score 
               + score.numpy_score 
               + score.pandas_score 
               + score.java_score 
               + score.project_score
    ...
    for att in attends:
        ...
        tot_rate += rate
    avg_attend_rate = tot_rate / total_student
